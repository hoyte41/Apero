	...
	class m_contact{
        ...
        */
		public function liste_message($numPage) {
			...
			$liste_message = $this->base_de_donnee->prepare('SELECT * FROM contact LIMIT ?, '.NB_ELEMENT_PAGE);
        ...
        */    
        public function get_message($id){
            $get_message = $this->base_de_donnee->prepare('SELECT * FROM contact WHERE id=?');
        ...
        */    
        public function add_message($nom, $prenom, $email, $sujet, $message, $dateEnvoie, $lu, $ip){
            $add_message = $this->base_de_donnee->prepare('INSERT INTO contact (nom, prenom, email, sujet, message, dateEnvoie, lu, ip) 
            values (?, ?, ?, ?, ?, ?, ?, ?) ');
        ...
        */    
        public function delete_message($id){
            $delete_message = $this->base_de_donnee->prepare('DELETE FROM contact WHERE id = ?');
        ...
        */    
        public function count_unread_message(){
            $count_unread_message = $this->base_de_donnee->prepare('SELECT COUNT(*) as nbMessagesNonLu FROM contact WHERE lu = 0');
        ...
        */    
        public function set_as_read($id){
            $set_as_read = $this->base_de_donnee->prepare('UPDATE contact SET  lu = 1 WHERE id = ?');
        ...
        */    
        public function set_as_unread($id){
            $set_as_read = $this->base_de_donnee->prepare('UPDATE contact SET  lu = 0 WHERE id = ?');
            $set_as_read->bindValue(1, $id, PDO::PARAM_INT);
            $set_as_read->execute();
        }
}
